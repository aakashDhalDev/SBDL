...
def get_contract(df):
    contract_title = array(when(~isnull("legal_title_1"),
                                struct(lit("lgl_ttl_ln1").alias("contractTitleLineType"),
                                    col("legal_title_1").alias("contractTitleLine")).alias("contractTitle")),
                            when(~isnull("legal_title_2"),
                                 struct(lit("lgl_ttl_ln2").alias("contractTitleLineType"),
    ...
    return df.select(
                        "account_id", get_insert_operation(col("account_id"), "contractIdentifier"),
...
def get_relations(df):
    return df.select(
                        ...
                        get_insert_operation(col("relation_start_date"), "partyRelationshipStartDateTime")
                    )
...
def apply_header(spark, df):
    ...
    event_df = header_df.hint("broadcast").crossJoin(df) \
                        .select(struct(expr("uuid()").alias("eventIdentifier"),
                                       col("eventType"), col("majorSchemaVersion"),
